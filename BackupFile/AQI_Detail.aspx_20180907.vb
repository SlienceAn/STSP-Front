...
Partial Class AQI_Detail
            ...
            While RD1.Read()
                ...
                s1 &= "<span class='label label-" & sli((v - 1) Mod sli.Length) & "'>" & v & "</span> " & "AQI" & "</li>" 
                    ...
                    While RD3.Read()
                        ...
                        Dim SQL20 As String
                        SQL20 = "Select Top 1 Value" & RD3("Colno") & " From STSP_S502T60 Where station='" & station_st & "' AND  Date_Time= '" & updatetime.AddHours(-1).ToString("yyyy/MM/dd HH:00:00") & "'  Order By Date_Time Desc"
                       'response.write(SQL20)
                        ...
                        If RD20.Read() Then
                            ...
                            s1 &= "<span class='pull-right' style='position:absolute; right:85px;'>" & val & "</span>"
                        Else
                            s1 &= "<span class='pull-right' style='position:absolute; right:85px;'>" & "數據處理中.." & "</span>"
                        End If
    ...
    Private Sub getEPAdata()
        ...
        'dim pDate as String = Now.Addhours(-1).ToString("yyyy/MM/dd HH:00:00")
        dim pDate as String = Now.ToString("yyyy/MM/dd HH:00:00")
        dim sql1 as string = "select top 4 [Date_Time],[Station],[AQI],[PM10],[PM10_AVG],[PM2.5],[PM2.5_AVG],[O3],[O3_8],[CO],[SO2],[NO2],[WD],[WS]" & _
            ...
            While RD1.Read()
                ...
                v += 1
                s1 &= "<li class='list-group-item'>"
                s1 &= "<span class='pull-right' style='position:absolute; right:85px;'>" & IIF(IsDBNull(RD1("PM10")),"---",RD1("PM10").ToString()) & "</span>"
                s1 &= "<span class='pull-right'>μg/m3</span>"            
                s1 &= "<span class='label label-" & sli((v - 1) Mod sli.Length) & "'>" & v & "</span>懸浮微粒</li>"
                v += 1
                s1 &= "<li class='list-group-item'>"
                s1 &= "<span class='pull-right' style='position:absolute; right:85px;'>" & IIF(IsDBNull(RD1("PM2.5")),"---",RD1("PM2.5").ToString()) & "</span>"
                s1 &= "<span class='pull-right'>μg/m3</span>"            
                s1 &= "<span class='label label-" & sli((v - 1) Mod sli.Length) & "'>" & v & "</span>細懸浮微粒</li>"        
                v += 1
                s1 &= "<li class='list-group-item'>"
                s1 &= "<span class='pull-right' style='position:absolute; right:85px;'>" & IIF(IsDBNull(RD1("O3")),"---",RD1("O3").ToString()) & "</span>"
                s1 &= "<span class='pull-right'>ppb</span>"            
                s1 &= "<span class='label label-" & sli((v - 1) Mod sli.Length) & "'>" & v & "</span>臭氧</li>"
                v += 1
                s1 &= "<li class='list-group-item'>"
                s1 &= "<span class='pull-right' style='position:absolute; right:85px;'>" & IIF(IsDBNull(RD1("CO")),"---",RD1("CO").ToString()) & "</span>"
                s1 &= "<span class='pull-right'>ppm</span>"            
                s1 &= "<span class='label label-" & sli((v - 1) Mod sli.Length) & "'>" & v & "</span>一氧化碳</li>"  
                v += 1
                s1 &= "<li class='list-group-item'>"
                s1 &= "<span class='pull-right' style='position:absolute; right:85px;'>" & IIF(IsDBNull(RD1("SO2")),"---",RD1("SO2").ToString()) & "</span>"
                s1 &= "<span class='pull-right'>ppb</span>"            
                s1 &= "<span class='label label-" & sli((v - 1) Mod sli.Length) & "'>" & v & "</span>二氧化硫</li>"  
                v += 1
                s1 &= "<li class='list-group-item'>"
                s1 &= "<span class='pull-right' style='position:absolute; right:85px;'>" & IIF(IsDBNull(RD1("NO2")),"---",RD1("NO2").ToString()) & "</span>"
                s1 &= "<span class='pull-right'>ppb</span>"            
                s1 &= "<span class='label label-" & sli((v - 1) Mod sli.Length) & "'>" & v & "</span>氮氧化物</li>"
                v += 1
                s1 &= "<li class='list-group-item'>"
                s1 &= "<span class='pull-right' style='position:absolute; right:85px;'>" & IIF(IsDBNull(RD1("WD")),"---",RD1("WD").ToString()) & "</span>"
                s1 &= "<span class='pull-right'>deg</span>"            
                s1 &= "<span class='label label-" & sli((v - 1) Mod sli.Length) & "'>" & v & "</span>小時風向</li>"     
                v += 1
                s1 &= "<li class='list-group-item'>"
                s1 &= "<span class='pull-right' style='position:absolute; right:85px;'>" & IIF(IsDBNull(RD1("WS")),"---",RD1("WS").ToString()) & "</span>"
