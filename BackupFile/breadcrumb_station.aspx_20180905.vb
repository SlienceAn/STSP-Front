    ...
    Sub getdata()
        ...
        station_code = Request("ST")
            ...
            If rd1.Read Then
                STtype = rd1("STtype")
                dTable = rd1("dTable")
                ST_Name = rd1("STName_TW")
                Dim updatetime As DateTime

                Dim sql2 As String
                sql2 = "Select Top 1 TableType,UpdateTime From STSP.dbo.STSP_StationTables "
                sql2 &= " Where STid='" & station_code & "' And IsGeneral=1 And UpdateTime Is Not null Order By sIndex"
                ...
                While RD3.Read()
                    ...
                    If station_code.Substring(0, 3) = "NPL" Then
                        SQL20 = "Select Top 1 Value" & RD3("Colno") & " From " & dTable & tt & " Where STid='" & station_code & "' Order By Date_Time Desc"
                    Else
                        SQL20 = "Select Top 1 Value" & RD3("Colno") & " From " & dTable & tt & " Where station='" & ST_Name & "'  Order By Date_Time Desc"
                    End If
				...
				RD3.close()
                tParameter.Text = s1
                tTotal.Text = "共" & v - 1 & "個測項，顯示來源：" & tt & " (" & updatetime.ToString(dFormat & " HH:mm:ss") & ")"
