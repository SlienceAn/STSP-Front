...
Partial Class breadcrumb_chart
    ...
    Sub ChartCreate()
        ...
        For c As Byte = 0 To col.Length - 1
            ...
            If STid.Substring(0, 3) <> "NPL" Then
                ...
                If STid = "臺南污水廠" Then
                    sql1 = "Select Colno,Parameter,Unit,ChartType,Decimalpoint,TableType From STSP.dbo.STSP_Parameter_water Where station='" & STid & "'And Colno='" & col(c) & "' and dp_no='" & dp_no_quest & "'"
            ...
            If RD2.Read() Then
                ...
                If STid.Substring(0, 3) <> "NPL" Then
                    ...
                    SqlStr &= "Date_Time >='" & sDate & "' And Date_Time<='" & eDate & "' " & pno & " Order By Date_Time"
                Else
                    SqlStr = "Select Convert(VARCHAR,Date_Time,120) AS Date_Time,Value" & col(c) & ",Status" & col(c) & " From " & dTable
                    SqlStr &= " Where  STid='" & STid & "' And "
                    SqlStr &= "Date_Time >='" & sDate & "' And Date_Time<='" & eDate & "' Order By Date_Time"
                End If
                ...
                While RD3.Read()
                    ...
                    Dim DT As String = RD3("Date_Time")
                    Dim Val As String
                    ...
                    yy = DT.Substring(0, 4)
                    mm = DT.Substring(5, 2) - 1
                    dd = DT.Substring(8, 2)
                    hh = DT.Substring(11, 2)
                    mn = DT.Substring(14, 2)
                    ss = DT.Substring(17, 2)
                    SerialStr &= "{ "
                ...
                If line(c) = "circle" Or line(c) = "square" Or line(c) = "diamond" Or line(c) = "triangle" Or line(c) = "triangle-down" Then
                    EachValueStr &= "       tooltip: { useHTML: true, pointFormat:'{point.y}', valueSuffix: ' " & unit & "' } ," & vbNewLine
                ...
                Else
                    ...
                    End If
                    EachValueStr &= "       tooltip: { valueSuffix: ' " & unit & "' } ," & vbNewLine
                End If
    ...
    Sub ChartParameter()
        ...
        If STid.Substring(0, 3) <> "NPL" Then
            ...
            If stid = "臺南污水廠" Then
                SqlStr = "Select Colno,Parameter,Unit,ChartType,TableType From STSP.dbo.STSP_Parameter_water Where station='" & stid & "' and dp_no='" & dp_no_quest & "' "
            ElseIf stid = "高雄污水廠" Then
                SqlStr = "Select Colno,Parameter,Unit,ChartType,TableType From STSP.dbo.STSP_Parameter_water Where station='" & stid & "'  and dp_no='" & dp_no_quest & "' "
            Else
