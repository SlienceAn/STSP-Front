...
Partial Class WaterDetail
    ...
    Sub get_water_data()
        ...
        For i = 0 To Tainan1_stids.Length - 1
            ...
            If RD1.Read() Then
                ...
                While RD2.Read()
                    ...
                    Dim SQL20 As String = "Select Top 1 Value" & RD2("Colno") & "  , Date_Time From " & dTable & tt & " Where station='" & ST_Name & "' and dp_no='" & Tainan1_stids(i) & "' Order By Date_Time Desc "
            ...
            Select Case Tainan1_stids(i)
                Case "D01"
                    ...
                    tTotalD01.Text = "共" & v - 1 & "個測項，顯示來源：5分鐘值  (" & dtt1.ToString(" HH:mm:ss") & ")"
                    tParameterD01.Text = s1
                Case "D02"
                    stationD02.Text = "D02 出水口監測結果"
                    tTotalD02.Text = "共" & v - 1 & "個測項，顯示來源：5分鐘值  (" & dtt2.ToString(dFormat & " HH:mm:ss") & ")"
        ...
        Else
            YT = dtt1
        End If
        examtime.Text = YT.ToString("yyyy/MM/dd  HH:mm")
        ' 以下處理高雄汙水廠資料
        s1 = ""
        v  = 1
        'Dim Kao_stids() As String = {"D01", "T01"}
        Dim Kao_stids() As String = {"D01"}
            For i = 0 To Kao_stids.Length - 1
                ...
                If RD1.Read() Then
                    ...
                    While RD2.Read()
                        ...
                        Dim SQL20 As String
                    SQL20 = "Select Top 1 Value" & RD2("Colno") & " ,Date_Time  From " & dTable & tt & " Where station='" & ST_Name & "' and dp_no='" & Kao_stids(i) & "' Order By Date_Time Desc "
                    Dim Com20 As New SqlCommand(SQL20, Conn)
                        Dim RD20 As SqlDataReader = Com20.ExecuteReader()
                        If RD20.Read() Then
            ...
            Station1.Text = "高雄污水廠 D01出水口"
        tTotal1.Text = "共" & v - 1 & "個測項，顯示來源：5分鐘值  (" & dtK.ToString(dFormat & " HH:mm:ss") & ")"
        tParameter1.Text = s1                    
            s1 &= "</ul>"
        examtime2.Text = dtK.ToString("yyyy/MM/dd  HH:mm")
