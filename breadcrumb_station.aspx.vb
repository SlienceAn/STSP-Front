...
Partial Class breadcrumb_station
    ...
    Sub getdata()
        ...
        station_code = Request("ST")
            ...
            If ST_Name = "臺南污水廠" Then
                sql3 = "Select DISTINCT Colno,Parameter,Unit,ChartType,Decimalpoint,TableType From STSP.dbo.STSP_Parameter_water Where station='臺南污水廠'  and dp_no ='" & dp_no_quest & "'  and Parameter<>'導電度' "
            ElseIf ST_Name = "高雄污水廠" Then
                sql3 = "Select Colno,Parameter,Unit,ChartType,Decimalpoint,TableType From STSP.dbo.STSP_Parameter_water Where station='高雄污水廠'  and dp_no ='" & dp_no_quest & "'   and Parameter<>'導電度' "
            End If
            ...
            Dim Com3 As New SqlCommand(sql3, Conn)
                ...
                Select Case RD3("Parameter")
                    Case "細懸浮微粒"
                        selStr = "  convert(decimal(9,0),Value" & RD3("Colno") & ") "
                    Case Else
                        selStr = " Value" & RD3("Colno")
                End Select

                If station_code.Substring(0, 3) = "NPL" Then
                    SQL20 = "Select Top 1 Value" & RD3("Colno") & " ,Date_Time From " & dTable & tt & " Where STid='" & station_code & "' Order By Date_Time Desc"
                Else
                    SQL20 = "Select Top 1  " & selStr & " ,Date_Time From " & dTable & tt & " Where station='" & ST_Name & "' " & pno & "  Order By Date_Time Desc"
                End If
                ...
                s1 &= "<span class='pull-right'>" & Replace(RD3("Unit"), "m3", "m³") & "</span>"
                RD20.Close()
    ...
    Function sMap(ByVal lat As Double, ByVal lng As Double, ByVal z As Byte) As String
        Dim ReStr As String
        If station_code = "Tainan_waste_water" Then
            Select Case dp_no_quest
                Case "D01"
                    lat = 23.108329
                    lng = 120.286136
                Case "D02"
                    ...
                    lng = 120.28017
            End Select
        ElseIf station_code = "Kaohsiung_waste_water" Then
            Select Case dp_no_quest
                Case "D01"
                    lat = 22.81926802
                    lng = 120.2829528
    ...
    Function sImg() As String 'Modify By Copan 2019/05/22
        Dim ReStr As String = ""
        ...
        If RD5.Read() Then
            If Not RD5("Img") = "" Then
                ReStr &= "<div class='flexslider'><ul class='slides'>"
                Dim ImageNames() As String = RD5("Img").ToString().Split(",")
                Dim ImageNotes() As String = RD5("Note").ToString().Split(",")
                Dim ImageNoteLength As String = ImageNotes.Length
                Dim NoteIndex As Integer = 0

                For Each Name As String In ImageNames
                    If station_code = "Tainan_waste_water" Then
                        If Name = "TNW.jpg" Then
                            ReStr &= "<li class='flex-active-slide'><a href ='image/Station_img/" + Name + "' data-fancybox='image' data-caption=''><img src='image/Station_img/" + Name + "' /></a></li>"
                        ElseIf Name.Contains("TNW_" + dp_no_quest) Then
                            ReStr &= "<li class='flex-active-slide'><a href ='image/Station_img/" + Name + "' data-fancybox='image' data-caption=''><img src='image/Station_img/" + Name + "' /></a></li>"
                        End If
                        If ImageNoteLength > 0 Then
                            If ImageNotes(0) <> " " Then
                                Dim InsertIndex As String = ReStr.IndexOf("<a href ='image/Station_img/" + Name)
                                If InsertIndex <> -1 Then
                                    ReStr = ReStr.Insert(InsertIndex, "<p class='flex-caption'>" + ImageNotes(NoteIndex) + "</p>")
                                    InsertIndex = ReStr.IndexOf("'><img src='image/Station_img/" + Name)
                                    If InsertIndex <> -1 Then
                                        ReStr = ReStr.Insert(InsertIndex, ImageNotes(NoteIndex))
                                    End If
                                End If
                            End If
                        End If
                        NoteIndex += 1
                    ElseIf station_code = "Kaohsiung_waste_water" Then
                        If Name = "KHW.jpg" Then
                            ReStr &= "<li class='flex-active-slide'><a href ='image/Station_img/" + Name + "' data-fancybox='image' data-caption=''><img src='image/Station_img/" + Name + "' /></a></li>"
                        ElseIf Name.Contains("KHW_" + dp_no_quest) Then
                            ReStr &= "<li class='flex-active-slide'><a href ='image/Station_img/" + Name + "' data-fancybox='image' data-caption=''><img src='image/Station_img/" + Name + "' /></a></li>"
                        End If
                        If ImageNoteLength > 0 Then
                            If ImageNotes(0) <> " " Then
                    ...
                    Else
                        ...
                        If ImageNoteLength > 0 Then
                            If ImageNotes(0) <> " " Then
                                For Each Note As String In ImageNotes
                                    Dim InsertIndex As String = ReStr.IndexOf("<a href ='image/Station_img/" + Name)
                                    ReStr = ReStr.Insert(InsertIndex, "<p class='flex-caption'>" + Note + "</p>")
                                    InsertIndex = ReStr.IndexOf("'><img src='image/Station_img/" + Name)
                                    ReStr = ReStr.Insert(InsertIndex, Note)
